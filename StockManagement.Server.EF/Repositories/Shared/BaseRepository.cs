...
{
    public class BaseRepository<T> : IContextStocksRepository<T> where T : class
    {
        ...
        public IEnumerable<T> FindAllIQuarable(Expression<Func<T, bool>> criteria, string[] includes = null)
        {
            IQueryable<T> query = _context.Set<T>();
        ...
        public async Task<T> FirstAsync(Expression<Func<T, bool>> expression = null, bool excludeDeleted = true)
        {
            T oTEntity = CreateNewInstance<T>();
            oTEntity = await GetAsQueryable(expression).FirstOrDefaultAsync();
